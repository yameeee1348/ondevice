...
module I2C_Slave(
   ...
   typedef enum logic [2:0] {
        ...
        TX_DATA   = 3'b100,
        SLAVE_ACK = 3'b101,
        M_ACK     = 3'b110   // 추가: 마스터의 ACK를 기다리는 상태
    } state_e;
        ...
        if (reset) begin
            ...
            case (state)
                ...
                SLAVE_ACK: begin
                    if (scl_falling) begin
                        sda_en  <= 1'b1;
                        sda_out <= 1'b0; // 슬레이브가 ACK(0) 구동
                    end else if (scl_rising) begin
                        state   <= RX_DATA; // 다음 데이터 대기 (연속 쓰기 지원)
                        bit_cnt <= 3'd7;
                    end
                end

                // ==========================================
                // [경로 2] 슬레이브 -> 마스터 (Read 전송)
                // ==========================================
                TX_DATA: begin
                    if (scl_falling) begin
                        sda_en  <= 1'b1;
                        sda_out <= tx_data[bit_cnt]; // 데이터 내보내기
                    end else if (scl_rising) begin
                        if (bit_cnt == 0) begin
                            state <= M_ACK; // 다 보냈으면 마스터의 응답을 기다리러 감!
                        end else begin
                            bit_cnt <= bit_cnt - 1;
                        end
                    end
                ...
                M_ACK: begin
                    if (scl_falling) begin
                        sda_en <= 1'b0; // 마스터가 ACK/NACK를 칠 수 있게 버스를 놔줌
                    end else if (scl_rising) begin
                        // 마스터가 1바이트만 읽고 NACK를 쳤으므로, 
                        // 다음 상태는 IDLE로 돌아가서 STOP Condition을 기다림
                        state <= IDLE;
                    end
                end
