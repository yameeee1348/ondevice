...
module top_uart (
    ...
    input  rst,
    input  uart_rx,
    // input [15:0] display_time_data,
    output uart_tx
    // output [3:0] cmd_tick,
    // output [2:0] cmd_switch
);
    wire w_b_tick, w_rx_done;
    wire [7:0] w_rx_data;  //, w_time_data;
    ...
    // wire mux_sel;
    // assign mux_sel = (w_sender_busy) ? 1'b1 : 1'b0;
    ...
    //     .sender_en(w_sender_en),
    //     .tx_done(w_tx_done),
    //     .display_time_data(display_time_data),
    //     .ascii_code(w_time_data),
    ...
    // ) U_MUX_2X1_9BIT (
    //     .sel(w_sender_busy),
    //     .i_sel0({w_rx_done, w_rx_data}),
    //     .i_sel1({w_sender_ready, w_time_data}),
    //     .o_mux(w_mux_out)
    // );
    ...
    //     .clk(clk),
    //     .rst(rst),
    //     .tx_start(w_mux_out[8]),
    //     .b_tick(w_b_tick),
    //     .tx_data(w_mux_out[7:0]),
    //     .tx_busy(),
    //     .tx_done(w_tx_done),
    //     .uart_tx(uart_tx)
    // );
...
module ascii_sender (
    ...
    parameter IDLE = 3'd0, DATA0 = 3'd1, DATA1 = 3'd2, DATA2 = 3'd3;
    parameter DATA3 = 3'd4, DATA4 = 3'd5, STOP = 3'd6;
    ...
    always @(*) begin
        ...
        case (c_state)
            IDLE: begin
                ...
                if (sender_en) begin
                    n_state = DATA0;
                    time_data_next = display_time_data;
                    ready_next = 1'b1;
                    busy_next = 1'b1;
                end
            end
...
module ascii_decoder (
    ...
    parameter SW_0 = 4'd6, SW_1 = 4'd7, SW_2 = 4'd8, STATE = 4'd9;
    reg [3:0] c_state, n_state;

    reg [7:0] data_r;
    reg [3:0] tick_reg, tick_next;
    reg [2:0] switch_reg, switch_next;
