...
module dht11_ctrl (
    ...
    reg [15:0] tick_cnt_reg, tick_cnt_next; // 19ms까지 카운트 가능하게 확장
    ...
    always @(*) begin
        ...
        case (c_state)
            ...
            START: begin
                dhtio_next = 1'b0; //MCU
                if (tick_1u) begin
                    if (tick_cnt_reg >= 18000) begin // 18ms 유지
                        tick_cnt_next = 0;
                        n_state       = WAIT;
                    end else begin
            ...
            DATA_C: begin // Measure High duration
                if (dhtio_sync2 == 1'b1) begin
                    if (tick_1u) tick_cnt_next = tick_cnt_reg + 1;
                end else begin
                    // Falling Edge: 판별 및 데이터 저장
                    data_next = {data_reg[38:0], (tick_cnt_reg >= 45) ? 1'b1 : 1'b0};
                    tick_cnt_next = 0;
...
module tick_gen_10usec (
    ...
    input      rst,
    output reg tick_10u // 10us마다 한 번씩 1이 되는 펄스
);
    ...
    always @(posedge clk, posedge rst) begin
        if (rst) begin
            ...
            if (counter_reg == F_COUNT - 1) begin
                counter_reg <= 0;
                tick_10u    <= 1'b1; // 1000번째 클락에서 펄스 발생
            end else begin
...
module tick_gen_1usec (
    input      clk,      // 100MHz 메인 클락
    input      rst,
    output reg tick_1u   // 1us마다 한 번씩 1이 되는 펄스
);
