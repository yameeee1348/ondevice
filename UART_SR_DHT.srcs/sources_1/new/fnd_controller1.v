...
module fnd_controller1 (
    ...
    input  [1:0] i_mode_sel,    // 이게 추가되어 있어야 함!
    input  [23:0] i_sr04_data,   // 이게 추가되어 있어야 함!
    input  [23:0] i_dht11_data,  // 이게 추가되어 있어야 함!
    ...
    reg [3:0] div5_cnt;
    reg [6:0] blink_msec;
    reg [23:0] r_selected_data;
    wire [6:0] msec_sel;
    wire       w_dot_onoff;
    // wire [7:0] fnd_data_internal;
    wire w_this_digit_dot;

    assign w_this_digit_dot = (w_digit_sel[1:0] == 2'b10) ? w_dot_onoff : 1'b0;

    //hour
    digit_spliter #(
        .BIT_WIDTH(5)
    ) U_HOUR_DS (
        .in_data (r_selected_data[23:19]),
    ...
    mux2x1 U_MUX_2x1 (
        .sel   ((i_mode_sel != 2'b00) ? 1'b1 : sel_display),
...
// 2. 데이터 선택 로직 (중복 제거 및 rst 처리 통합)
    always @(*) begin
        if (rst) begin 
            r_selected_data = 24'd0; // 리셋 시 0으로 초기화
        end else begin
            case (i_mode_sel)
                2'b01:   r_selected_data = {11'd0, i_sr04_data[5:0], 7'd0}; 
                2'b10:   r_selected_data = {i_dht11_data[23:18], i_dht11_data[15:10], 13'd0};
                default: r_selected_data = fnd_in_data; 
            endcase
        end
    end
assign fnd_digit = (w_digit_sel[1:0] == 2'b00) ? 4'b1110 :
                       (w_digit_sel[1:0] == 2'b01) ? 4'b1101 :
                       (w_digit_sel[1:0] == 2'b10) ? 4'b1011 : 4'b0111;
    

    bcd U_BCD (
    .bcd(w_mux_2x1_out),
    .i_dot(w_this_digit_dot), // 위에서 만든 신호를 전달
    .fnd_data(fnd_data)       // 최종 출력
);
 ...
 // 블링크용 타이머
    always @(posedge clk or posedge rst) begin
        if (rst) begin
            div5_cnt <= 4'd0; blink_msec <= 7'd0;
        end else if (w_1khz) begin
            if (div5_cnt == 4'd9) begin
                div5_cnt <= 4'd0;
                blink_msec <= (blink_msec == 7'd99) ? 7'd0 : blink_msec + 1;
            end else div5_cnt <= div5_cnt + 1;
        end
    end
...
module bcd (
    ...
    always @(*) begin
        case (bcd)
            4'd0: seg_data = 7'b000_0001; // 0
            4'd1: seg_data = 7'b100_1111; // 1
            4'd2: seg_data = 7'b001_0010; // 2
            4'd3: seg_data = 7'b000_0110; // 3
            4'd4: seg_data = 7'b100_1100; // 4
            4'd5: seg_data = 7'b010_0100; // 5
            4'd6: seg_data = 7'b010_0000; // 6
            4'd7: seg_data = 7'b000_1111; // 7
            4'd8: seg_data = 7'b000_0000; // 8
            4'd9: seg_data = 7'b000_0100; // 9
            default: seg_data = 7'b111_1111; // 꺼짐
        endcase
        // [중요] seg_data와 i_dot을 결합하여 fnd_data에 할당
        fnd_data = {seg_data, ~i_dot};
    end
endmodule
